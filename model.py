...
class Model:
    ...
    def garch11_controlled_model(self, param, endog):
        ...
        s[2] = st.variance(r[0:3])
        for i in range(3, n):
            s[i] = (omega + alpha * r[i - 1] ** 2 + beta * (s[i - 1])) * math.exp(
                gmon * endog[i, 2] - gjan * endog[i, 3] - gend * endog[i, 4])  # GARCH(1,1) model
        logL = -((-np.log(s) - r ** 2 / s).sum())
        return logL

    def garch11_full_model(self, param, endog):
        omega, alpha, beta, a, b, monday, jan, end, gmon, gjan, gend, gdp = param
        r = endog[:, 0] - a - b * endog[:, 1] - monday * endog[:, 2] - jan * endog[:, 3] - end * endog[:,
                                                                                                 4] - gdp * endog[:, 5]
        n = len(r)
        s = np.ones(n) * 0.01
        s[2] = st.variance(r[0:3])
    ...
    def fitted_values(self, endog, est_params, model):
        if (model == "simple"):
            ...
            self.estimated = a + b * endog[:, 1] + monday * endog[:, 2] + jan * endog[:, 3] + end * endog[:,
                                                                                                    4] + gdp * endog[:,
                                                                                                               5]
            e = endog[:, 0] - self.estimated
            n = len(e)
